...
WORDLIST_FILE        = os.path.join(BASE_DIR, "alwird_words.json")
ANSWERS_FILE         = os.path.join(BASE_DIR, "alwird_answers.json")
CACHE_FILE_FULL      = os.path.join(BASE_DIR, "alwird_cache.pkl")
CACHE_FILE_ANSWERS   = os.path.join(BASE_DIR, "alwird_cache_answers.pkl")
...
def load_answers() -> list[str]:
    """Load the answer-only word list (the real solutions the game can pick)."""
    ...
    cleaned = []
    for w in words:
        w = ARABIC_DIACRITICS.sub("", w)
        if len(w) == WORD_LEN:
            cleaned.append(w)
    print(f"  [✓] Loaded {len(cleaned):,} answer words")
    return cleaned
...
def run_interactive_game(all_words: list[str], answers: list[str], legacy: bool, hard: bool) -> None:
    ...
    opener, _  = get_best_opener(answers)
...
def run_benchmark(all_words: list[str], answers: list[str], legacy: bool, hard: bool) -> None:
    ...
    opener, _ = get_best_opener(answers)
    sample    = random.sample(answers, min(n, len(answers)))
    results   : list[int] = []
...
def main() -> None:
    ...
    while True:
        active_cache = CACHE_FILE_ANSWERS if answers_mode else CACHE_FILE
        print(_menu(active_cache))
        ...
        # ── Switch answers / full cache + candidate pool ──────────────────────
        if choice == "7":
            if not answers_mode:
        ...
        if choice == "5":
            CACHE_FILE = CACHE_FILE_ANSWERS if answers_mode else CACHE_FILE_FULL
            words, cand = load_pools()
            if cand:
                run_warmup(cand)
            input("\n  Press Enter to return to menu …")
            continue
        ...
        if choice in SHORTCUTS:
            ...
            if interactive is None:
                run_benchmark(words, cand, legacy=legacy, hard=hard)
            elif interactive:
                run_interactive_game(words, cand, legacy=legacy, hard=hard)
            else:
                run_auto_game(words, cand, legacy=legacy, hard=hard)
            input("\n  Press Enter to return to menu …")
            continue
        ...
        if choice in ("1", "2", "3", "4"):
            ...
            if choice in ("1", "2"):
                ...
                if hard is None:
                    continue
                if choice == "1":
                    run_interactive_game(words, cand, legacy=False, hard=hard)
                else:
                    run_auto_game(words, cand, legacy=False, hard=hard)
  ...
  └─────────────────────────────────────────┘""")
                ...
                if sub in legacy_map:
                    ...
                    if interactive:
                        run_interactive_game(words, cand, legacy=True, hard=hard)
                    else:
                        run_auto_game(words, cand, legacy=True, hard=hard)
                else:
                    continue
  ...
  └─────────────────────────────────────────┘""")
                ...
                if sub in bench_map:
                    lg, hd = bench_map[sub]
                    run_benchmark(words, cand, legacy=lg, hard=hd)
                else:
                    continue
...
def main() -> None:
    ...
    while True:
        ...
        if choice == "5":
            CACHE_FILE = CACHE_FILE_ANSWERS if answers_mode else "alwird_cache.pkl"
            words, cand = load_pools()
            if cand:
                run_warmup(cand)
            input("\n  Press Enter to return to menu …")
